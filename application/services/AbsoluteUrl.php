<?php
class Ops_Service_AbsoluteUrl
{
    public function makeAbsolute($baseUrl, $relativeUrl)
    {
        // If relative URL has a scheme, clean path and return.
        $r = $this->_splitUrl($relativeUrl);
        if (FALSE === $r) {
            return FALSE;
        }
        if (!empty($r['scheme'])) {
            if (!empty($r['path']) && $r['path'][0] == '/')
                $r['path'] = $this->_removeDotSegments($r['path']);
            return $this->_joinUrl($r);
        }

        // Make sure the base URL is absolute.
        $b = $this->_splitUrl($baseUrl);
    ...
    */
    protected function _splitUrl($url, $decode=FALSE)
    {
    ...
    */
    public function _joinUrl($parts, $encode=FALSE)
    {
        if ($encode) {
            if (isset($parts['user']))
